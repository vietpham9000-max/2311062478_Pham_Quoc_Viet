...
const AdminCoursesPage = () => {
  ...
  const handleDelete = async (course: Course) => {
    if (!window.confirm(`Bạn có chắc muốn xóa ${course.courseCode} - ${course.courseName}?`)) return;
    setMessage("");
    setActionError("");
    try {
      ...
      if (courses.length === 1 && currentPage > 0) setCurrentPage((page) => page - 1);
      else await refetch();
    } catch (error: unknown) {
  ...
  return (
    <div className="app-container">
      <header className="app-header"><h1>Quản trị môn học</h1><p>Thêm, sửa, xóa và tìm kiếm danh sách môn học</p></header>
      <main className="app-content">
        {message && <div className="alert success-alert">{message}</div>}
        {actionError && <div className="alert error-alert">{actionError}</div>}
        <CourseForm editingCourse={editingCourse} onSuccess={handleFormSuccess} onCancelEdit={() => setEditingCourse(null)} />
        <SearchBox value={keyword} onChange={handleSearch} />
        <div className="card">
          <CourseList courses={courses} state={state} errorMessage={errorMessage} refetch={refetch}
            onEdit={(course) => {
