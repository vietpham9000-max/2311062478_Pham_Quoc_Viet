package com.phamquocviet.courseservice.security;
...
@Component public class JwtAuthFilter extends OncePerRequestFilter {
 private final javax.crypto.SecretKey key; public JwtAuthFilter(@Value("${jwt.secret}") String s){key=Keys.hmacShaKeyFor(Base64.getDecoder().decode(s));}
 @Override protected void doFilterInternal(HttpServletRequest r,HttpServletResponse p,FilterChain c)throws ServletException,IOException{
  String h=r.getHeader("Authorization"); if(h!=null&&h.startsWith("Bearer "))try{Claims x=Jwts.parser().verifyWith(key).build().parseSignedClaims(h.substring(7)).getPayload(); String role=x.get("role",String.class); SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken(x.getSubject(),null,List.of(new SimpleGrantedAuthority("ROLE_"+role))));}catch(JwtException|IllegalArgumentException e){p.setStatus(401);p.setContentType("application/json");p.getWriter().write("{\"status\":401,\"error\":\"Unauthorized\",\"message\":\"JWT không hợp lệ hoặc đã hết hạn\"}");return;} c.doFilter(r,p);
 }}
